...
public:
    ...
    int getImageSizewidthpadding() const {
        const auto size = 3 * m_width + getPaddingSize(m_width*3 ,4);
    ...
    void savePixel(std::ofstream& file)
    {
        for (int row = m_height - 1; row >= 0; --row) {
            ...
            addPadding(file);
          /*  for (int i = 0; i < getPaddingSize(m_width * 3, 4); ++i)
            {
                uint8 val = 0;
...
private:
    ...
    int getPaddingSize(int  val,int multiple) const {
        int padding = (multiple - (val % multiple)) % multiple;
        return padding;
    }

    // Ajoute le padding à la fin d'une ligne
    void addPadding(std::ofstream& file) {
        int padding = getPaddingSize(m_width * 3,4);
